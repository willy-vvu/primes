...
In the model, we have the professor grade one paper, called \emph{X}. The professor then distributes all the student's papers randomly among the students, while making sure that no student receives his or her own paper. Every student is also given a copy of \emph{X}.
...
\subsection{Problems}
The goal for this model was to make it more realistic while still making it simple to understand, and have a simply solution that would be easy for students to understand. We now have a model that works very well in an unrealistic scenario, so we decided that this solution was fine, and moved on to a new model. 
...
This provides a very different scenario than the original model, because now students will be able to figure out which papers are calibrated. However, we still wanted to try and generalize the previous solution and apply it to the new model, so we came up with the following scheme.
...
In the new mechanism, we went with a completely different strategy, that we wanted to work better than the previous one.
...
So assuming that the student's partner gives the paper a 100, which is the best thing they could do if they don't grade, then
	If you don't grade the paper, you don't use any effort. So the student's happiness will H(the student's grade on the original assignment). Contribution score = 0
	If you do grade, you use 1 unit of effort by default. But, the student's happiness will be H(the student's grade on the original assignment) + 3. Contribution score = 1
	
If the grader's partner gives the paper its true grade:
	If the grader doesn't grade the assignment, you will give it a 100 and so the grader's contribution score is -1. So their happiness is H(your original grade) - 4.
